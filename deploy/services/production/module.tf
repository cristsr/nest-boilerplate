...
module "service" {
  ...
  ecs_cluster_id             = data.terraform_remote_state.replaceme.outputs.production.cluster_id
...
EOF
  ...
  service_discovery_namespace = data.terraform_remote_state.replaceme.outputs.production.namespace
}
